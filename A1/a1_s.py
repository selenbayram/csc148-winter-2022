...
class GameBoard:
    ...
    # _raccoons:
    #   the raccoons in this game
    # _characters:
    #   all the characters in this game
    ...
    _raccoons: Optional[List[Raccoon]]
    _characters: Optional[List[Character]]
    ...
    def at(self, x: int, y: int) -> List[Character]:
        ...
        if (not self.on_board(x, y)) or (self._characters is None):
            return lst
        else:
    ...
    def check_game_end(self) -> Optional[int]:
        ...
        trapped_raccoons = 0
        for raccoon in self._raccoons:
            if (raccoon.get_char() == 'R') or (not raccoon.check_trapped()):
                self.ended = False
            if raccoon.check_trapped():
                trapped_raccoons += 1
        self.ended = True and self.ended
    ...
    def adjacent_bin_score(self) -> int:
        ...
        #     u = (b[0], b[1] - 1)
        #     d = (b[0], b[1] + 1)
        #     if (l not in bin_tiles) and (r not in bin_tiles) and\
        #         (u not in bin_tiles) and (d not in bin_tiles):
        #         count += 1
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if self.board.on_board(new_x, new_y):
            ...
            if len(self.board.at(new_x, new_y)) == 0:
                # tile is empty
                self.x = new_x
                self.y = new_y
                return True
                ...
                next_bin = self.board.at(new_x, new_y)[0]
...
class Player(TurnTaker):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        new_x = self.x + direction[0]
        new_y = self.y + direction[1]
        if (self.board.on_board(new_x, new_y)) and \
                (not isinstance(self.board.at(new_x, new_y), Raccoon)):
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        True
        """
        return ((not self.move(UP)) and (not self.move(RIGHT)) and
                (not self.move(LEFT)) and (not self.move(DOWN)))
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        new_y = self.y + direction[1]
        if self.board.on_board(new_x, new_y) and \
                (not isinstance(self.board.at(new_x, new_y),
            ...
            if self.get_char() == '@':
                ...
                return False
            elif len(self.board.at(new_x, new_y)) == 0:
    ...
    def take_turn(self) -> None:
        ...
        for d in DIRECTIONS:
            if (self.board.on_board(self.x + d[0], self.y + d[1])) and \
                    (grid_board[self.y + d[1]][self.x + d[0]] == 'O' or
                     grid_board[self.y + d[1]][self.x + d[0]] == '-'):
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        #     distance_to_garbage_can = []
        #     for d in DIRECTIONS:
        #         new_x = self.x + d[0]
        #         new_y = self.y + d[1]
        #         count = 0
        #         if (isinstance(self.board.at(new_x, new_y)[0],
        #                        GarbageCan)) and \
        #            (len(self.board.at(new_x, new_y)) != 2):
        #             # the tile 1 tile away from the smart raccoon is occupied by
        #             # empty garbage can
        #             count += 1
        #         while (len(self.board.at(new_x, new_y)) == 0)\
        #             or (isinstance(self.board.at(new_x, new_y)[0],
        #                            Player)):
        #             # the tile 1 tile away from current position of the smart
        #             # raccoon is either empty or occupied by the player
        #             new_x += d[0]
        #             new_y += d[1]
        #             if (isinstance(self.board.at(new_x, new_y)[0],
        #                            GarbageCan)) and \
        #                     (len(self.board.at(new_x, new_y)) != 2):
        #                 # the tile 1 tile away from the smart raccoon is
        #                 # occupied by empty garbage can
        #                 count += 1
        #         distance_to_garbage_can.append(count)
        #
        #     for c in distance_to_garbage_can:
        #         pass
