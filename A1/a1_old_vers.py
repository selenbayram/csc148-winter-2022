...
# get_shuffled_directions not used!!
# explicitly in the docstring!!!
# some helper functions are missing docstring examples
# playing the game: SmartRaccoon disappears after it opens a garbage can (?)
# convert all the public methods into private methods

# SO use a master dictionary
# each character stored in this master dictionary
# keys are master[self]
# values
...
class GameBoard:
    ...
    def add_to_dict(self, r: Character, x: int, y: int) -> None:
        """
    ...
    def adjacent_bin_score(self) -> int:
        ...
        5
        """
        # MAYBE USEFUL FROM PIAZZA
        # Since you keep tracking the recycling bins you've already counted in a
        # stack, your length of counted_stack is increasing. Conversely, your
        # length of list of unchecked recycling bins is decreasing. Following
        # this 'procedure, eventually you will reach a base case.
        # I think you should be able to figure it out what the base case is.

        # DONE
        # Go through all the tiles that have recycling bins and, for each one,
        # see how many it is adjacent to. Then we can return the biggest result
        # we find. But how do we find the number of recycling bins adjacent to
        # a particular recycling bin? We can start accumulating a count by
        # adding in all the recycling bins directly adjacent.

        # WORK ON THIS
        # But then we need to remember to count all the recycling bins that are
        # adjacent to those recycling bins. We can do that by recording those
        # recylcing bins somewhere (perhaps in something like a stack or a
        # queue) and coming back to check them later. Be careful not to double
        # count though. You can avoid double-counting by keeping track of which
        # recycling bins you have already considered.

        # max_count = self.helper_adjacent(self._bin_c[0])
        # for bin_to_cons in self._bin_c[1:]:
        #     c = self.helper_adjacent(bin_to_cons)
        #     if c > max_count:
        #         max_count = c
        #
        # return max_count
        ...
        for rec in self._bins:
            ...
            for rec2 in self._bins:
                if u == [rec2.x, rec2.y] and u not in self._considered:
                    self._considered.append(u)  # add counted bins to considered
                    self._considered_coords.append([rec2.x, rec2.y])
                    s.append(u)  # and stack
                    count += 1
                if d == [rec2.x, rec2.y] and d not in self._considered:
                    self._considered.append(d)
                    self._considered_coords.append([rec2.x, rec2.y])
                    s.append(d)
                    count += 1
                if le == [rec2.x, rec2.y] and le not in self._considered:
                    self._considered.append(le)
                    self._considered_coords.append([rec2.x, rec2.y])
                    s.append(le)
                    count += 1
                if r == [rec2.x, rec2.y] and r not in self._considered:
    ...
    def helper_adjacent(self, direct: list) -> int:
        ...
        #
        # count = 1
        # if self.at(l[0], l[1])[0] != 'B' \
        #         and self.at(u[0], u[1])[0] != 'B' \
        #         and self.at(r[0], r[1])[0] != 'B' \
        #         and self.at(d[0], d[1])[0] != 'B':
        #     return count
        #
        # elif self.at(l[0], l[1])[0] == 'B' \
        #         or self.at(u[0], u[1])[0] == 'B' \
        #         or self.at(r[0], r[1])[0] == 'B' \
        #         or self.at(d[0], d[1])[0] == 'B':
        #     count += 1
        #     if self.at(l[0], l[1])[0] == 'B':
        #         b = [l[0] - 1, y]
        #         c = self.helper_adjacent(b)
        #         count += c
        #     if self.at(r[0], r[1])[0] == 'B':
        #         b = [r[0] + 1, y]
        #         c = self.helper_adjacent(b)
        #         count += c
        #     if self.at(u[0], u[1])[0] == 'B':
        #         b = [u[0], y - 1]
        #         c = self.helper_adjacent(b)
        #         count += c
        #     if self.at(d[0], d[1])[0] == 'B':
        #         b = [d[0], y + 1]
        #         c = self.helper_adjacent(b)
        #         count += c
        #
