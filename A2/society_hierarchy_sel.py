...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        True
        """
        lst = merge(self._subordinates, [subordinate])
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self.cid == cid:
            ...
            for sub in self._subordinates:
                res = sub.get_citizen(cid)
                if isinstance(res, Citizen):
                    return res
            return None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        if not self._subordinates:
            ...
            for sub in self._subordinates:
                all_subs = merge(all_subs, sub.get_all_subordinates())
                f = merge(all_subs, [sub])
            return f
